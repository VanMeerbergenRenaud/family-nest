...
<label for="{{ $name }}" class="relative w-full flex items-center cursor-pointer">
    <input id="{{ $name }}"
           model="{{ $model }}"
           type="checkbox"
           class="h-4 w-4 cursor-pointer transition-all appearance-none rounded border border-slate-300 checked:bg-slate-800 checked:border-slate-800 dark:checked:bg-gray-300"
        {{ $checked ? 'checked' : '' }}
        {{ $attributes }}
    />
