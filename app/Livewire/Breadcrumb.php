...
class Breadcrumb extends Component
{
    ...
    protected function buildBreadcrumb(): void
    {
        ...
        // Add section index for grouped routes
        $this->addSectionSegment($currentRoute);

        // Add current page segment
        $this->segments->push([
            'name' => $currentRoute,
    ...
    protected function addSectionSegment(string $currentRoute): void
    {
        $prefix = collect($this->groupPrefixes)
            ->first(fn($p) => Str::startsWith($currentRoute, $p));
